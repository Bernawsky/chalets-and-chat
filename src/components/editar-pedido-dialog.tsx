...
import { editarPedido } from "@/lib/pedidos.functions";
import {
  LINK_GRUPO,
  gerarMensagemEdicao,
  normalizarHorario,
...
export function EditarPedidoDialog({ pedido, onClose }: { pedido: Pedido; onClose: () => void }) {
  const queryClient = useQueryClient();
  const editar = useServerFn(editarPedido);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      return editar({ data: { id: pedido.id, saudacao: saudacao.trim(), unidades: payload } });
    },
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-foreground/50 p-4 backdrop-blur-sm">
      <div className="my-8 w-full max-w-3xl rounded-2xl border border-border bg-background p-5 shadow-xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {unidades.map((u) => (
            <UnidadeCard
                  ...
                  prev.map((it) =>
                    it.id === id
                      ? {
                          ...it,
                          dietas: ativo
                            ? Array.from(new Set([...(it.dietas ?? []), key]))
                            : (it.dietas ?? []).filter((d) => d !== key),
                        }
                      : it,
                  ),
                )
              ...
              onLimpar={(id) =>
                setUnidades((prev) => prev.filter((it) => it.id !== id))
              }
            />
          ))}
        </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <Button onClick={salvar} disabled={mutation.isPending} className="gap-2">
            <Save className="size-4" aria-hidden="true" />
            {mutation.isPending ? "Salvando..." : "Salvar e enviar"}
          </Button>
        </div>
      </div>
    </div>
